package org.cloudcrawler.domain.crawler.robotstxt.parser;
...
import org.cloudcrawler.domain.crawler.robotstxt.rules.SimpleRobotRules;
...
public class SimpleRobotRulesParser extends BaseRobotsParser {
    ...
    /**
     * Figure out directive on line of text from robotstxt.txt file.
    ...
    // Max # of warnings during parse of any one robotstxt.txt file.
    private static final int MAX_WARNINGS = 5;

    // Max value for crawl delay we'll use from robotstxt.txt file. If the value is greater
    ...
    public BaseRobotRules failedFetch(int httpStatusCode) {
        ...
        if ((httpStatusCode >= 200) && (httpStatusCode < 300)) {
            ...
            // Actually treat all (including forbidden) as "no robotstxt.txt", as that's what Google
    ...
    public BaseRobotRules parseContent(String url, byte[] content, String contentType, String robotName) {
        ...
        while (keepGoing && lineParser.hasMoreTokens()) {
            ...
            // page for robotstxt.txt. We could do more sophisticated processing here to better
            // handle bad HTML, but that's a very tiny percentage of all robotstxt.txt files.
            if (hasHTML) {
